...
| 文件 | 作用 | 通常什么时候修改 |
| --- | --- | --- |
| `backend/Dockerfile` | 使用 Maven/Java 21 多阶段构建测试并打包后端，以非 root 用户运行 JRE 镜像 | 调整后端镜像、Java 运行时或容器启动方式时 |
| `backend/.dockerignore` | 排除本地 target、真实环境变量、编辑器状态和日志 | Docker 构建上下文需要增删文件时 |
| `backend/maven-settings.xml` | 让后端 Docker 构建通过阿里云 Maven 镜像下载 Central 依赖 | Docker 构建的 Maven 镜像策略变化时 |
...
| `backend/src/main/resources/db/migration/V5__update_kubernetes_containerization_progress.sql` | 把学习记录的下一步更新为镜像加载和 Helm Chart | 已执行后不再修改 |
...
| 修改后端镜像 | `backend/Dockerfile` | `.dockerignore`、Maven 镜像配置、Kubernetes values |
| 修改后端线上部署 | `.woodpecker/backend.yml` | `deploy-backend.sh`、Compose |
